    ...
    <div class="filter-criteria">
        <el-card>
            ...
            <el-collapse-item name="recommend" title="推荐条件">
             ...
             <!--   <el-form-item label="涨幅范围并加入自选(%)：" prop="roseMin">
                  <el-row :gutter="8">
                    <el-col :span="11">
                      <el-form-item prop="roseMin"> 
                        <el-input 
...
import type {RecommendConfig, StockAnalysisType} from "../../utils/dataType";
...
const stockList = computed<StockAnalysisType[]>(() => stStore.analysisList);
...
//获取推荐股票code
const getRecommendedStockCodes = (stocks: StockAnalysisType[]) => {
    const list = stocks.map(stock => stock.code);
    console.log('Recommended stock codes:', list);
  return list;
};
// 提交表单
const handleSubmit =async () => {
  ...
  filterForm.value?.validate(async (valid) => {
      if (valid) {
        recommendCfg.rows = getRecommendedStockCodes(stockList.value);
        // 2. 验证通过：打印表单结果（primaryCfg 即完整表单数据）
        console.log('推荐表单提交结果：', JSON.parse(JSON.stringify(recommendCfg))); // 深拷贝避免引用影响
        const res = await apiFuns.loadRecommendStockAnalysis(recommendCfg);
        if (res != undefined) {
            ElMessage.success(`共筛选出 ${res.length} 支符合条件的股票`);
            stStore.setAnalysisList(res);
            console.log('stockList in store:', stStore.analysisList);
        }
      } else {
        // 3. 验证失败：提示错误
