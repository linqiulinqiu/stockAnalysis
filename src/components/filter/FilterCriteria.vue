        ...
        <el-card>
          ...
          <!-- 一级筛选条件（数值输入项） -->
            ...
            <div class="form-actions">
              <el-button @click="handleAll">全选</el-button>
...
// 全选二级筛选条件
const handleAll = () => {
  primaryCfg.screenOne = true;
  primaryCfg.screenTwo = true;
  primaryCfg.screenThree = true;
  primaryCfg.screenTen = true;
  primaryCfg.screenFifteen = true;
  primaryCfg.screenSixteen = true;
  ElMessage.success('二级筛选条件已全部选中');
};
 // 提交表单
const handleSubmit =async () => {
  ...
  filterForm.value?.validate(async (valid) => {
    if (valid) {
      ...
      const res = await apiFuns.loadStocks(primaryCfg);
      if (res != undefined) {
